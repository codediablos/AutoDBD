...
class DbdDaemon(Daemon):
    ...
    def kimai_login(self):
        ...
        if not html.find('Access denied!') == -1:
            self.logger.error('kimai login fail')
            return False
        else:
            self.logger.info('kimai login success')
...
#            if state_index in GENERAL_STATE:
    ...
    def dbd(self):
        ...
        foods_cfg = self.get_config().get('core', 'food4days')
        choice_foods = int(self.get_config().get('core', 'food'))
        try :
            choice_foods = int(foods_cfg.split(',')[datetime.datetime.today().weekday()])
        except:
            pass
        ...
        if choice_foods >= len(foods):
            choice_foods = 0

        # print foods

        values = dict(authenticity_token=self.authtok, food=foods[choice_foods]['value'], location=location_cfg, save_item=u'我也要訂 !!')
